...
<script type="text/javascript">
	...
	var req_list = function(){
		...
		post("${path}/charge/list", {pageNumber:pageNumber}, function(data) {
			...
			for(var i = 0;i<list.length;i++){
				...
				var html = 
					'		<div class="list-group-item" onclick="toInfo('+row.id+')">'+
					'			<div class="left-t">'+
					'				<div>'+
					'					<div>计价月份:'+row.month+'</div><br>'+
					'					<div>金额:<span style="color: red">'+row.money+'&yen;</span></div>'+
					'				</div>'+
					'			</div>'+
					'			<div class="right-t">'+
					'				<div>'+
					'					<div>'+status+'</div><br>'+
					'					<div>用量:'+row.dosage+'</div>'+
					'				</div>'+
					'			</div>'+
					'			<div class="clear"></div>'+
					'		</div>';
	...
	var toInfo = function(id){
		window.location.href = "${path}/charge/info?id="+id+"&open_id=${open_id}";
	}
</script>
</html>
